...
module.exports = {
  ...
  joinInPrivateCommunity: async (payload) => {
    try {
      return await communityModel.updateOne(
        { _id: payload.communityId },
        { $push: { users: { userId: payload.id, pending: true } } },
      );
    } catch (error) {
      return error;
    }
  ...
  checkUserinCommunity: async (payload) => {
    try {
      const response = await communityModel.find(
        { _id: payload.communityId, users: { $elemMatch: { userId: payload.userId } } },
      );
  ...
  checkIsuserAdmin: async (payload) => {
    try {
      const response = await communityModel.findOne(
        { _id: payload.communityId, admin: payload.userId },
      );
  ...
  communityPosts: async (payload) => {
    try {
      ...
      return await postModel.find(
        { communityId: payload.communityId, deleted: false },
      ).sort({ createdAt: -1 });
    } catch (error) {
      return error;
    }
  },
  userPosts: async (payload) => {
    try {
      return await postModel.find({ userId: payload.id, deleted: false }).sort({ createdAt: -1 });
    } catch (error) {
      console.log(error);
      return error;
    }
